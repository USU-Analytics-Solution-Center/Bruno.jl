...
@testset "Statistical consistency check for $v" for v in
    ...
    @test isapprox(mean(results), beta; atol = 0.05)
end
...
@testset "Optimal Block Length test" begin
    ...
    @test isapprox(opt_block_length(ar, Stationary), 6.3085; atol = 0.001)
    @test isapprox(opt_block_length(ar, CircularBlock), 7.2214; atol = 0.001)
end

@testset "$bstype Dimensional check for nSimulation > 1" for bstype in [
    Stationary,
    MovingBlock,
    CircularBlock,
]
