...
@testset verbose = true "type constructor tests" begin
    ...
    @testset "Test constructor limits for $v" for v in (MovingBlock,
                                                        CircularBlock, 
                                                        Stationary)
        # test for size of input_data
        @test_throws ErrorException BootstrapInput{v}(; input_data = [1], n = 10,
            block_size = 1)
        # test constraint for block_size
        @test_throws ErrorException BootstrapInput{v}(; input_data = [1,2,3], n = 10,
            block_size = 5)
        @test_throws ErrorException BootstrapInput{v}(; input_data = [1,2,3], n = 0, 
            block_size = 2)
    end
end